...
var books = [
    { 
        ...
        text            : 'Google Images', 
        url             : 'https://www.google.co.uk/imghp',
        icon            : 'picture',
        height          : 260, 
        width           : 42, 
        font            : "Open Sans",
...
$(function(){
    ...
    imgs.leather.src = 'images/leather.png';
    imgs.canvas.src = 'images/canvas.png';
});
...
function addBooks (books) {
    ...
    var i = 0;
    while(totalWidth < docWidth){
        book = $.extend(true, $.extend(true, {}, defaultBook), bookspecs[Math.floor(Math.random()*bookspecs.length)]);
        book.text = randomWords[(i%randomWords.length)];
        var next = " " + randomWords[((i+1)%randomWords.length)];
        if((book.height < 160 && (book.text + next).length < 8)){
            console.log(book.height);
            book.text += next;
        } else if((book.height > 160 && (book.text + next).length < 13)) {
            console.log(book.height);
            book.text += next;
        }
        totalWidth += book.width;
        if(totalWidth > docWidth){
            book.width = docWidth - (totalWidth - book.width);
        }
        $('#bookcase').append(makeBook(book));
        i += 2;
    }
...
function makeBook (o){
    ...
    if(options.url){
        var $text = $('<a href="' + options.url + '" class="booktext" title="">' + options.text + '</a>');
    } else {
